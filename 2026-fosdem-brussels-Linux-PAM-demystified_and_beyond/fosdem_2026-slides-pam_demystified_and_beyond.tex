% Copyright (C) 2012-2026 Dmitry V. Levin <ldv@strace.io>
...
\pgfdeclareimage[height=0.6cm]{left-corner-logo}{FOSDEM_logo.pdf}
\pgfdeclareimage[height=0.6cm]{right-corner-logo}{pamela.jpg}
\pgfdeclareimage[height=4cm]{title-logo}{FOSDEM_logo_hex.pdf}
...
\title{\Huge Linux-PAM demystified and beyond}
\author{\LARGE Dmitry~Levin}
\date{\Large Brussels, 2026}
...
%%%%%%%
\begin{frame}{\Large The setuid helper issue \hfill [\insertframenumber/\inserttotalframenumber]}
\Large
\begin{block}{pam\_unix: traditional approach}
\begin{itemize}
	\item a helper is needed to access /etc/shadow
	\item unix\_chkpwd helper is setuid-root
\end{itemize}
\end{block}
\begin{block}{pam\_tcb approach (since 2001)}
\begin{itemize}
	\item /etc/shadow \(\longrightarrow\) /etc/tcb/user/shadow
	\item tcb\_chkpwd helper is setgid-shadow
\end{itemize}
\end{block}
\begin{block}{pam\_unix: client-server approach (since 2025)}
\begin{itemize}
	\item unix\_chkpwd helper is unprivileged
	\item communicates with pwaccessd via a Unix domain socket
\end{itemize}
\end{block}
\end{frame}
...
\begin{itemize}
	\item journalctl -u {\it service-name}
\end{itemize}
\end{block}
